...
### Fixed
- **TOC Scroll Behavior Shadowed by Search Modal**: The `scrollToShot()` function in `main.js` (used by TOC item clicks) was being silently overwritten by the identically-named function in `search-modal.js` due to global script scope. This caused TOC clicks to use the search modal's scroll behavior (center scroll, orange glow) instead of the intended TOC behavior (top-aligned scroll, background flash, auto-close drawer). The TOC function has been renamed to `tocScrollToShot()` to eliminate the collision, restoring the original intended behavior for TOC navigation while preserving the search modal's independent scroll behavior.
...
- **Modular JavaScript Architecture**: The monolithic `main.js` (2,595 lines) has been split into three focused modules:
  - `main.js` (1,547 lines) — app core: state management, project dialogs, theme, shot grid rendering, drag-and-drop uploads, shot CRUD, tooltips, notifications
  - `toc.js` (217 lines) — Table of Shots panel: UI creation, positioning (docked/drawer), toggle, rendering, filtering, scroll-to-shot, IntersectionObserver, archived section collapse
  ...
  All functions were moved verbatim with zero logic changes. Script load order in `index.html`: `main.js` → `toc.js` → `modals.js` → `visual-reorder.js` → `search-modal.js` → `modal-behavior.js`.
